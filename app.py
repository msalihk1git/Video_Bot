...
firebase_admin.initialize_app(cred, {"storageBucket": "videoprocessingapi.appspot.com"})
...
def upload_to_firebase(video_path):
    ...
    blob = bucket.blob("videoprocessingapi.appspot.com/output_video.mp4")
...
def process_video():
    try:
        ...
        if modified_frames:
            ...
            firebase_video_url = upload_to_firebase(output_video_path)

            # Emit a socket event with the Firebase Storage URL
            socketio.emit('video_generated', {'video_url': firebase_video_url, 'phone_number': sender_phone_number})

            return jsonify({"status": "success", "message": "Video generation completed", "video_url": firebase_video_url})
        else:
            return jsonify({"status": "error", "message": "No frames to process"})
